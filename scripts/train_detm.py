...
if __name__ == "__main__":
    ...
    parser.add_argument("--min_word_occurrence", dest="min_word_occurrence", type=int, default=0, help="Words occuring less than this number of times throughout the entire dataset will be ignored")
            ...
            with gzip.open(getattr(args, name), "rt") as ifd:
                for line in ifd:
                    ...
                    for word in full_text_words:
                        ...
                        docs.append(word)                    
                    for subdoc_num, subdoc in enumerate(split_doc(docs, args.max_subdoc_length)):
                        utokens = set()
                        total_subdocs += 1
                        for t in subdoc:
                            utokens.add(t)
...
#>>>>>>> fe5fbfeebd540fca44991b9cb0ff002f5360ec34
    ...
    for name in list(subdoc_counts.keys()):
        subdoc_counts[name] = [dict([(k, v) for k, v in s.items() if k != "window"] + [("window", window_transform[s["window"]])]) for s in subdoc_counts[name] if s["window"] in windows_to_keep]
        window_counts[name] = {
            window_transform[k] : v for k, v in window_counts[name].items() if k in windows_to_keep
        }
    ...
    # embeddings = np.zeros((len(token2id), 300))
    # words_found = 0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        #if numpy.isnan(acc_loss):
        #    logger.info("NaN loss, stopping!")
        #    break
